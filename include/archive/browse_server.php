<?php
	...
	$sql = "
		SELECT s.*, 
            ...
            (
                SELECT CONCAT(sr.quantity, '__', sr.date, '__', sr.comment) 
                FROM stock_registry sr
				WHERE (sr.stock_id = s.stock_id)
                ORDER BY sr.`date` DESC
                LIMIT 1
	...
	if (!$qry) {
		...
		if ($qry->num_rows > 0) {
			...
			while( $obj = $qry->fetch_object() ) {
				...
				$arr = explode("__",$obj->registry_data);

				$rows[$i]['DT_RowId'] = $obj->stock_id;
				$rows[$i]['code'] = $obj->code;
				$rows[$i]['description'] = htmlentities($obj->description);
				$rows[$i]['stock'] = htmlentities($arr[0]);
//				$rows[$i]['date'] = ($arr[1]!='' ? set_formatted_date($arr[1]) : '');
